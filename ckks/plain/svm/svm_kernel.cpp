...
int main() {
    ...
    ofstream fout;
    fout.open("svm_kernel_plaintext.txt", ios::out | ios::app); // Open the file in append mode
    if (!fout) {
    ...
    for (int d = 5; d <= 15; d++) { // Test dimensions from 5 to 15
        ...
        for (int i = 0; i < n; i++) {
            pinverse_product[i] = pinverse(kernel[i][i] / divisors[i]) / divisors[i];
        }
...
vector<vector<double>> kernelize(vector<vector<double>> x, int n, int d) {
    vector<vector<double>> kernel(n, vector<double>(n, 0.0));

    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            kernel[i][j] = pinner_product(x[i], x[j], d);
