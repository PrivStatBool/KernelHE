...
//		bootsAND(&E[i], &AA[i], &temp[1], bk);
//		bootsAND(&F[i], &D[i], &temp[0], bk);
		bootsMUX(&res[i], &temp[1], &AA[i], &D[i], bk);	
	}
...
void HomMultiReal(LweSample* res, const LweSample* a, const LweSample* b, const int length, const int int_length, const TFheGateBootstrappingCloudKeySet* bk) {
	
	LweSample* temp = new_gate_bootstrapping_ciphertext_array(2, bk->params);
	LweSample* A = new_gate_bootstrapping_ciphertext_array(length, bk->params);
	LweSample* B = new_gate_bootstrapping_ciphertext_array(length, bk->params);
	LweSample* C = new_gate_bootstrapping_ciphertext_array(length+1, bk->params);
	LweSample* D = new_gate_bootstrapping_ciphertext_array(length+1, bk->params);
	...
	bootsCONSTANT(&C[length-1], 0, bk);		
	bootsCOPY(&C[length], &A[length-1], bk);		
	
	for(int i = 1; i < length-1; i++){
		if(i < length-int_length){
			bootsCONSTANT(&D[0], 0, bk);			
			for(int j = 0; j < length; j++){
				bootsAND(&D[j+1], &A[j], &B[i], bk);}

			HomAdd(C, C, D, length+1, bk);
			HomLShift(C, C, length+1, 1, bk);
		}
			...
			for(int j = 0; j < count; j++){
				bootsCONSTANT(&D[j], 0, bk);}
			for(int j = 0; j < length-count+1; j++){
				bootsAND(&D[j+count], &A[j], &B[i], bk);}
	...
	bootsCOPY(&C[length-1], &C[length], bk);		
	
	HomTwosCompliment(D, C, length, bk);
...
//		bootsAND(&E[i], &AA[i], &temp[1], bk);
//		bootsAND(&F[i], &D[i], &temp[0], bk);
		bootsMUX(&res[i], &temp[1], &C[i], &D[i], bk);	
	}
...
//	HomAdd(res, E, F, length, bk);
	...
	delete_gate_bootstrapping_ciphertext_array(length, A);
	delete_gate_bootstrapping_ciphertext_array(length, B);
	delete_gate_bootstrapping_ciphertext_array(length+1, C);
	delete_gate_bootstrapping_ciphertext_array(length+1, D);
}
